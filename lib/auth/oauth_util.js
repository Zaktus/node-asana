...
function parseOauthResultFromTitle(title) {
  var sParams = title.split(' '), // length = 2
      status =  decodeURIComponent(sParams[0]),
      res = {status:status},
      params = decodeURIComponent(sParams[1]).split('&');

  params.forEach(function (e) {
    var keyVal = e.split('=');
    if (keyVal.length == 2) {
       var key = (keyVal[0] === 'code') ? 'token' : keyVal[0];
       res[key] = keyVal[1];
    }
  });
  return res;
...
module.exports = {
  ...
  parseOauthResultFromTitle: parseOauthResultFromTitle,
